...
#define MIP_SRC_FORMAT rgba16f

#define TARGET_IMAGE_TYPE_2D 0
#define TARGET_IMAGE_TYPE_2D_ARRAY 1
#define USE_TARGET_IMAGE_TYPE  TARGET_IMAGE_TYPE_2D
...
layout(set=0, binding=0, MIP_SRC_FORMAT) uniform TARGET_IMAGE_TYPE imgDst[MAX_MIPS_SPD_GEN + 1];

struct MipMapGenCfg{
    uint mipsCount;
    uint numWorkGroups;// use dispatch z as total slice count
    uint reduceFunctionType;
...
layout(set = 0, binding = 1) coherent buffer spdGlobalAtomicBuffer {
    uint counter[MAX_SLICE_SPD_PROCESS];
}SpdGlobalAtomic;
...
{
    if(MIP_GEN_CFG.reduceFunctionType == REDUCE_TYPE_MAX){
        AF4 ret = AF4(
        ...
        return ret;
    }else if(MIP_GEN_CFG.reduceFunctionType == REDUCE_TYPE_MIN){
        AF4 ret = AF4(
