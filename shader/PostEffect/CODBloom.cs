...
DECL_BUFFER_STD140_BEG(BloomConfig)
    float BloomStrength;
    float Threshold;        
    float Radius;               //Up sample parameter, controls the uv step. 
    float Padding3;
DECL_BUFFER_STD140_BEG
...
RESOURCE_DECL_BEG(1)
    ...
    SLOT_CONST_BUFFER(4,BloomConfig,Cfg)
RESOURCE_DECL_END
...
vec3 TentFilterSample(vec2 uv, vec2 uvStepPerPixSmallTex , texture2D sampledTexture , sampler samp){
    ...
    vec2 uvStep = uvStepPerPixSmallTex * BLOOM_CFG.Radius;
    vec3 out = vec3(0.);
    for(int i = 0; i < 9; ++ i){
...
vec3 Tap13Sample(vec2 curUV,vec2 uvStep,texture2D sampledTexture,sampler textureSampler){
    ...
    for(int i = 0;i < 13; ++i){
        ...
        tap13SampledPoint[i] = texture(Sampler2D(sampledTexture,textureSampler)).rgb;
    }
...
void UpSampleMain(){
    ...
    ivec2 mipn_1Size = textureSize(GetTexture(MipN_1));
    ivec2 curPixel = gl_GlobalInvocationID.xy;
    vec2 curUV = vec2(curPixel) / vec2(imgSize);
    vec2 uvStepPerPixMipN_1 = vec2(1.) / vec2(mipn_1Size);
    vec3 tentOut = TentFilterSample(curUV,uvStepPerPixMipN_1,GetTexture(MipN_1), GetSampler(BilinearSampler));
    imageStore(GetRWTexture(MipN),curPixel);
}